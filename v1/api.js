...
class Api {
  ...
  filter(dbname, params) {
    const urlBase = this.config.url;
    const url = `${urlBase}/f/${dbname}/?filter=${params}`;
    const config = {
      method: "GET",
      headers: this.headers(),
    };
  ...
  update(dbname, uid, params) {
    const urlBase = this.config.url;
    const url = `${urlBase}/u/${dbname}/?uid=${uid}`;
    const config = {
      method: "POST",
      body: JSON.stringify(params),
      headers: this.headers(),
